...
3\. Update `APP\_IMAGE` in the production `.env` file
...
bash scripts/rollback.sh docker.io/legiang2090/final-tier2-app:sha-OLD\_COMMIT
...
app\_http\_requests\_total

app\_http\_request\_duration\_seconds
...
sum(rate(app\_http\_requests\_total\[1m]))
...
sum(rate(app\_http\_requests\_total{status\_code=\~"5.."}\[5m])) or vector(0)
...
histogram\_quantile(0.95, sum(rate(app\_http\_request\_duration\_seconds\_bucket\[5m])) by (le))
...
APP\_IMAGE=docker.io/legiang2090/final-tier2-app:sha-xxxxxxxx

APP\_VERSION=local-dev

DATABASE\_URL=postgres://finaluser:password@db:5432/finaldb

POSTGRES\_DB=finaldb

POSTGRES\_USER=finaluser

POSTGRES\_PASSWORD=change-me

GRAFANA\_USER=admin

GRAFANA\_PASSWORD=change-me
